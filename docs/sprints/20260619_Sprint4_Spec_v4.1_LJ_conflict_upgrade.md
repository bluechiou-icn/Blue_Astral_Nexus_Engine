...
> 📌 **Status Update（2026-06-19 17:00 Taipei，本檔上 git 後追加）**
> 遠端 main 已於 2026-06-18 完成 Sprint 4 v4.0 Step 1（祿忌交戰 severity 外框 + hover tooltip，commit `fad8ee6`）。
> 本 v4.1 規格定位調整為「**在 v4.0 已實作基礎上的 UPGRADE patch**」，非從零實作：
> - F1（移除 v4.0 紅 / 橘外框）→ 改為「重構 v4.0 已有的外框邏輯為 overlay 閃爍格」
> - E1（severity 1-6）→ 改為「擴充 v4.0 已有的 severity enum」
> - 進入點：先讀 `fad8ee6` commit 的 chart-render.js 改動，理解現況再升級
...
### 3.1 範例（generic — 遵 CLAUDE.md Rule 3 不引真實命盤）

以 canonical 範例參數 `2000-01-01 06:00 男` 推導後的虛構案例：
