...
/**
 * 六干四化明細：列出六層天干各自的四化飛落，供 Agent 做完整因果鏈論斷。
 * 僅呈現事實，不做計分（計分由 scoreHourPalace 負責）。
 *
 * @param {object[]} palaces     本命十二宮
 * @param {string}   mingBranch  流時命宮地支
 * @param {object}   layerStems  { 生年, 大限, 流年, 流月, 流日, 流時 } 天干
 * @returns {object} 六層各含 { stem, 化祿, 化權, 化科, 化忌 }，每化含 star/landsPalace/hitsMinPalace/hitsTriad
 */
function buildMutagens6Layers(palaces, mingBranch, layerStems) {
  const LAYER_ORDER   = ["生年","大限","流年","流月","流日","流時"];
  const MUTAGEN_TYPES = ["化祿","化權","化科","化忌"];
  const triad         = new Set(triadBranches(mingBranch));

  const result = {};
  for (const layer of LAYER_ORDER) {
    const stem = layerStems[layer];
    if (!stem) { result[layer] = { stem: null, unavailable: true }; continue; }
    const m = mutagenStarsByStem(stem);
    if (!m)   { result[layer] = { stem, unavailable: true }; continue; }

    const detail = { stem };
    for (const type of MUTAGEN_TYPES) {
      const star       = m[type];
      const branch     = starBranch(palaces, star);
      const palace     = branch ? palaces.find(p => p.branch === branch) : null;
      detail[type] = {
        star,
        landsBranch:    branch,
        landsPalace:    palace?.name || null,
        hitsMinPalace:  branch === mingBranch,
        hitsTriad:      branch ? triad.has(branch) : false,
      };
    }
    result[layer] = detail;
  }
